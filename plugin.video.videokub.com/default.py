...
class VideoKub():
    def __init__(self):
        ...
        self.url = 'http://www.videokub.online/'
    ...
    def menu(self):
        ...
        self.index('http://www.videokub.online/latest-updates/', 1)
    ...
    def genres(self):
        ...
        url = 'http://www.videokub.online/categories/'
    ...
    def search(self, keyword, unified, page):
        ...
        if keyword:
            ...
            url = 'http://www.videokub.online/search/'+str(page)+'/?q=%s' % (keyword)
            request = urllib2.Request(url)
            request.add_header('User-agent', 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.71 Safari/537.36')
            response = urllib2.urlopen(request)
            resp = response.read()
